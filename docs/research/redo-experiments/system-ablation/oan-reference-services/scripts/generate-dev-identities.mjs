...
const LEGACY_CRYPTO_SUITE = "Ed25519Sha256Legacy";
const LEGACY_HASH_ALGORITHM = "SHA-256";
const LEGACY_VERIFICATION_METHOD_TYPE = "Ed25519VerificationKey2020";
const LEGACY_PROOF_TYPE = "Ed25519Signature2020";
...
function loadExistingKeyMaterial(definition) {
  const keypairPath = path.join(dataDir, definition.dataPath, "keys", "keypair.json");
  if (!fs.existsSync(keypairPath)) {
    return null;
  }
  const value = JSON.parse(fs.readFileSync(keypairPath, "utf8"));
  if (!value?.privateKeyJwk?.d || !value?.publicKeyJwk?.x) {
    return null;
  }
  return value;
}

function legacySignatureInput(value) {
  return Buffer.from(sha256Hex(canonicalJson(value)), "utf8");
}

function buildVerificationMethod(did, keyId, publicKeyMultibase, publicJwk) {
  return {
    id: keyId,
    type: LEGACY_VERIFICATION_METHOD_TYPE,
    controller: did,
    cryptoSuite: LEGACY_CRYPTO_SUITE,
...
function buildProof(privateKey, creator, verificationMethod, createdAt, payload) {
  return {
    type: LEGACY_PROOF_TYPE,
    creator,
    created: createdAt,
    proofPurpose: "assertionMethod",
    proofValue: crypto.sign(null, legacySignatureInput(payload), privateKey).toString("base64url"),
    cryptoSuite: LEGACY_CRYPTO_SUITE,
...
function createIdentity(definition) {
  const existing = loadExistingKeyMaterial(definition);
  const privateKey = existing
    ? crypto.createPrivateKey({ key: existing.privateKeyJwk, format: "jwk" })
    : crypto.generateKeyPairSync("ed25519").privateKey;
  const publicKey = existing ? crypto.createPublicKey(privateKey) : crypto.createPublicKey(privateKey);
  const publicJwk = existing?.publicKeyJwk ?? publicKey.export({ format: "jwk" });
  const privateJwk = existing?.privateKeyJwk ?? privateKey.export({ format: "jwk" });
  ...
  const publicKeyMultibase = `z${base58Encode(publicKeyRaw)}`;
  ...
  const didDocument = {
    ...
    id: did,
    verificationMethod: [
      buildVerificationMethod(did, keyId, publicKeyMultibase, publicJwk),
    ],
...
function signEvent(rootPrivateKey, actorDid, event) {
...
for (const event of bulletinEvents) {
  ...
  signedEvents.push(signEvent(rootPrivateKey, identityById.root.did, { ...event, previousHash }));
}
...
for (const identity of identities) {
  ...
  const metadataHash = sha256Hex(canonicalJson(metadata));
  const packageClaims = {
  ...
  const verifiedPackage = {
    ...
    rootProof: {
      rootDid: identityById.root.did,
      bulletinEventHash: null,
      signature: null,
      packageClaims,
      proof: buildProof(
        rootPrivateKey,
        identityById.root.keyId,
        identityById.root.keyId,
        createdAt,
        packageClaims,
      ),
